...
async def dub(
    ...
    try:
        ...
        # Initialize strict_segment_timing with default from config (may be overridden by form data if available)
        if 'strict_segment_timing' not in locals():
...
# ============================================
# BULK DUBBING - CORRECTED IMPLEMENTATION  
# All critical bugs fixed from code review
# ============================================

# Bulk job tracking with thread-safe updates
@dataclass  
class BulkJob:
    batch_id: str
    total: int
...
async def bulk_worker(worker_id: int):
    ...
    while True:
        ...
        try:
            ...
            async with job.lock:
                job.processing += 1
                job.videos[video_index]['status'] = 'processing'
...
async def bulk_run(
    ...
    if files:
        for file in files:
            if file.filename:
                file_path = await persist_uploaded_file(file, UPLOADS_DIR)
