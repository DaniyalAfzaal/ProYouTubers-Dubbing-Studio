...
if __name__ == "__main__":
    try:
        ...
        with contextlib.redirect_stdout(sys.stderr):
            ...
            # Use large-v3 for best accuracy (latest WhisperX model)
            whisper_model = extra.get("model_name") or os.getenv("WHISPERX_MODEL_NAME", "large-v3")
            batch_size    = int(os.getenv("WHISPERX_BATCH_SIZE", extra.get("batch_size", 1)))
            # Auto-select compute type: float16 on GPU, int8 on CPU
            default_compute = "int8" if not torch.cuda.is_available() else "float16"
            compute_type  = extra.get("compute_type") or os.getenv("WHISPERX_COMPUTE_TYPE", default_compute)
